          ...
          <div className="grid grid-cols-3 gap-20 p-6 mt-20">
            {services.map((service) => (
              <Link href={`/service_help/${service.ref}`} key={service.id}>
                <a className="flex flex-col items-center transition-transform lg:hover:scale-110 hover:text-sky-700 transform-gpu">
                  <img
                    src={service.image}
                    alt={service.name}
                    className="cursor-pointer w-12 h-12 md:w-28 md:h-28 object-cover rounded mb-4"
                  />
